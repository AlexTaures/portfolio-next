    ...
    const skillsMapped = (params: skillParameters, key: number) => {
        return (
            <div key={key} className='p-2 m-2 border-gray-500 border-dotted border-2 rounded-xl hover:scale-105 hover:shadow-2xl ease-in duration-100'>
                ...
                <a href={params.url} className='project-image relative'>
                    <img src={params.image} alt="project-image"/>
                    <p className="h-full w-full absolute top-0 left-0 bg-transparent text-transparent flex justify-center items-center text-[2em] blur-sm hover:backdrop-blur-sm hover:bg-black/40 hover:text-white hover:blur-none ease-out duration-500">
                        Click to open
                    </p>
                </a>
